...
static void ClosePipeFromProgram(CopyState cstate);
static bool CopyStreamReadLine(CopyState cstate);
static bool CopyStreamReadLineText(CopyState cstate);
static int	CopyStreamReadAttributesText(CopyState cstate);
static int	CopyStreamReadAttributesCSV(CopyState cstate);
static Datum CopyStreamReadBinaryAttribute(CopyState cstate,
						int column_no, FmgrInfo *flinfo,
						Oid typioparam, int32 typmod,
						bool *isnull);
static bool CopyStreamGetInt16(CopyState cstate, int16 *val);
 ...
 */
static bool
CopyStreamGetInt16(CopyState cstate, int16 *val)
{
	uint16		buf;
	   ...
	   uint64 *processed)
{
	...
	if (stmt->relation)
	{
		...
		rel = table_openrv(stmt->relation,
						  (is_from ? RowExclusiveLock : AccessShareLock));

		rte = addRangeTableEntryForRelation(pstate, rel, (is_from ? RowExclusiveLock : AccessShareLock), NULL, false, false)->p_rte;
 ...
 */
void
ProcessStreamCopyOptions(ParseState *pstate,
				   CopyState cstate,
				   bool is_from,
				   List *options)
{
		  ...
		  List *options)
{
	...
	/* Extract options from the statement node tree */
	ProcessStreamCopyOptions(pstate, cstate, is_from, options);
...
CopyStreamFrom(CopyState cstate)
{
	...
	*(estate->es_result_relations) = resultRelInfo;
...
#else
	myslot = ExecInitExtraTupleSlot(estate, tupDesc, &TTSOpsVirtual);
#endif
	...
	for (;;)
	{
		...
		if (!NextCopyStreamFrom(cstate, econtext, values, nulls, &loaded_oid))
			break;
		...
		slot = myslot;
		ExecStoreBufferHeapTuple(tuple, slot, InvalidBuffer);
 ...
 */
void
EndCopyStreamFrom(CopyState cstate)
{
 ...
 */
static void
ClosePipeFromProgram(CopyState cstate)
{
	int			pclose_rc;
 ...
 */
bool
NextCopyStreamFromRawFields(CopyState cstate, char ***fields, int *nfields)
{
	int			fldct;
	bool		done;
	...
	done = CopyStreamReadLine(cstate);
	...
	if (cstate->csv_mode)
		fldct = CopyStreamReadAttributesCSV(cstate);
	else
		fldct = CopyStreamReadAttributesText(cstate);
 ...
 */
bool
NextCopyStreamFrom(CopyState cstate, ExprContext *econtext,
			 Datum *values, bool *nulls, Oid *tupleOid)
{
	...
	if (!cstate->binary)
	{
		...
		if (!NextCopyStreamFromRawFields(cstate, &field_strings, &fldct))
			return false;
	...
	else
	{
		...
		if (!CopyStreamGetInt16(cstate, &fld_count))
		{
		...
		if (file_has_oids)
		{
			...
			cstate->cur_attname = "oid";
			loaded_oid =
				DatumGetObjectId(CopyStreamReadBinaryAttribute(cstate,
														 0,
		...
		foreach(cur, cstate->attnumlist)
		{
			...
			cstate->cur_attname = NameStr(att->attname);
			i++;
			values[m] = CopyStreamReadBinaryAttribute(cstate,
												i,
												&in_functions[m],
												typioparams[m],
												att->atttypmod,
 ...
 */
static bool
CopyStreamReadLine(CopyState cstate)
{
	bool		result;
	...
	result = CopyStreamReadLineText(cstate);
 ...
 */
static bool
CopyStreamReadLineText(CopyState cstate)
{
	char	   *copy_raw_buf;
	int			raw_buf_ptr;
	int			copy_buf_len;
 ...
 */
static int
CopyStreamReadAttributesText(CopyState cstate)
{
 ...
 */
static int
CopyStreamReadAttributesCSV(CopyState cstate)
{
	char		delimc = cstate->delim[0];
 ...
 */
static Datum
CopyStreamReadBinaryAttribute(CopyState cstate,
						int column_no, FmgrInfo *flinfo,
						Oid typioparam, int32 typmod,
						bool *isnull)
{
