 ...
 *
 * This file is pipelinedb's new json.c based on "ca3b37487be333a1d241dab1bbdd17a211a88f43"
 *
 * IDENTIFICATION
...
composite_to_jsonb(Datum composite, JsonbInState *result)
{
	...
	for (i = 0; i < tupdesc->natts; i++)
	{
		...
		if (isnull)
		{
			...
			jsonb_categorize_type(TupleDescAttr(tupdesc, i)->atttypid,
								  &tcategory, &outfuncoid);
 ...
 */
PG_FUNCTION_INFO_V1(combinable_jsonb_agg_transfn);
Datum
combinable_jsonb_agg_transfn(PG_FUNCTION_ARGS)
{
...
PG_FUNCTION_INFO_V1(combinable_jsonb_agg_finalfn);
Datum
combinable_jsonb_agg_finalfn(PG_FUNCTION_ARGS)
{
	JsonbAggState *arg;
	JsonbInState result;
	Jsonb	   *out;
...
/*
 * combinable_jsonb_object_agg_transfn
 */
PG_FUNCTION_INFO_V1(combinable_jsonb_object_agg_transfn);
Datum
combinable_jsonb_object_agg_transfn(PG_FUNCTION_ARGS)
{
	MemoryContext oldcontext,
				aggcontext;
	JsonbInState elem;
...
PG_FUNCTION_INFO_V1(combinable_jsonb_object_agg_finalfn);
Datum
combinable_jsonb_object_agg_finalfn(PG_FUNCTION_ARGS)
{
	JsonbAggState *arg;
	JsonbInState result;
	Jsonb	   *out;
 ...
 * jsonb_agg_combine
 */
PG_FUNCTION_INFO_V1(jsonb_agg_combine);
Datum
 ...
 */
PG_FUNCTION_INFO_V1(jsonb_object_agg_combine);
Datum
...
PG_FUNCTION_INFO_V1(jsonb_agg_serialize);
Datum
jsonb_agg_serialize(PG_FUNCTION_ARGS)
{
	JsonbValue *jsonb;
...
PG_FUNCTION_INFO_V1(jsonb_agg_deserialize);
Datum
jsonb_agg_deserialize(PG_FUNCTION_ARGS)
{
	Jsonb *jsonb;
	JsonbIterator *it;
