 ...
 */
PG_FUNCTION_INFO_V1(pipeline_get_worker_querydef);
Datum
pipeline_get_worker_querydef(PG_FUNCTION_ARGS)
{
	...
	if (!RangeVarIsContView(rv) && !RangeVarIsContTransform(rv))
		elog(ERROR, "\"%s\" is not a continuous view or transform", TextDatumGetCString(name));

	query = RangeVarGetContWorkerQuery(rv);
 ...
 */
PG_FUNCTION_INFO_V1(pipeline_get_combiner_querydef);
Datum
